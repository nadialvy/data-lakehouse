...
def transform_status(df):
    print("Transforming status dimension...")

    # Ambil hanya kolom status unik
    df = df[['status']].drop_duplicates()

    # Transform: trim + title case
    df['status'] = df['status'].str.strip().str.title()

    # Surrogate key
    last_id = get_max_surrogate('dim_status', 'status_id')
    df['status_id'] = range(last_id + 1, last_id + 1 + len(df))

    return df[['status_id', 'status']]
...
# === MAIN ===
def run_etl_status():
    df = extract_table("log_frs")
    dim_status = transform_status(df)
    load_table(dim_status, "dim_status")
